...
### Log-sum-exp 技巧 <!-- learning-atlas: the-log-sum-exp-trick -->
...
所得概率完全相同，计算过程却更安全；这种处理是保证正确性的必要条件。
...
当数值有界、精度比范围更重要时，推理通常首选 float16；当范围比精度更重要时，训练通常首选 bfloat16。因此 TPU 以及现代 NVIDIA GPU（A100、H100）都原生支持 bfloat16。
...
梯度裁剪是训练安全机制。如果不使用它，一个异常批次产生的巨大梯度就足以毁掉数周的训练成果。
...
2. **寻找精度极限。** 在 Python 中，找出满足 `1.0 + x == 1.0` 的最小正 float32 值 `x`。这个值就是机器 epsilon。验证它与 `numpy.finfo(numpy.float32).eps` 一致。
