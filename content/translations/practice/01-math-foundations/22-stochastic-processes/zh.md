...
许多 AI 系统都处理随时间演化的结构化随机性，其中每一步都取决于此前发生的事情。

语言模型一次生成一个词元。每个词元都依赖之前的上下文；模型输出概率分布，从中采样，再继续生成。整个序列构成一个随机过程。
...
这种条件独立性称为马尔可夫性质，因此可以用转移矩阵 P 描述整个系统的动力学：
...
当温度 T = 0 时，系统执行纯梯度下降。在高温下，它近似为随机游走。在适当温度下，粒子会探索能量景观，并在低能量区域停留更长时间。
...
用 P 反复乘初始分布。经过足够多次迭代后，无论从哪里开始，它都会收敛到平稳分布；这相当于用幂方法寻找占优左特征向量。
...
随机过程也是现代 AI 系统内部的计算机制。调整 LLM 温度会改变序列采样过程；训练扩散模型是在学习反转类似布朗运动的过程；执行贝叶斯推断则是在构造一条收敛到后验分布的链。
...
- **Ho、Jain、Abbeel（2020）**——《Denoising Diffusion Probabilistic Models》。奠定现代扩散模型基础的 DDPM 论文，其中清晰推导了前向和逆向马尔可夫链。
