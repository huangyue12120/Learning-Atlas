...
### 图拉普拉斯矩阵 <!-- learning-atlas: the-graph-laplacian -->
...
4. **Fiedler 值对应的特征向量（Fiedler 向量）揭示最佳划分。** 取正值的节点进入一组，取负值的节点进入另一组，这种方法称为谱聚类。
...
一轮 GNN 消息传递会为每个节点计算新特征：先对邻居特征加权平均，再乘以权重矩阵。堆叠多轮，即可将信息传播得更远。
