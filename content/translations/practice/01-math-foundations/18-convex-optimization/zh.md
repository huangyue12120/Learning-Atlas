...
互补松弛是其中的关键洞见：要么约束是活跃的（g_i = 0，解位于边界上），要么乘子为零（该约束不起作用）。不影响解的约束，其 lambda = 0。
...
**真正的障碍是鞍点，而非局部最小值。** 对一个有 n 个参数的函数，鞍点在不同方向上兼有正曲率和负曲率。在高维空间的随机临界点处，全部 n 个特征值均为正（即局部最小值）的概率大约为 2^(-n)。几乎所有临界点都是鞍点，而 SGD 的噪声有助于逃离这些点。
