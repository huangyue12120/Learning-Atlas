...
这并不是一道脑筋急转弯，而是贝叶斯定理。每一个垃圾邮件过滤器、每一项医疗诊断，以及每一个量化不确定性的机器学习模型，都在使用完全相同的推理方式。你从一个信念出发，看到证据，然后更新信念。
...
不足 1%。先验占据主导。当一种疾病很罕见时，即使检测相当准确，大部分阳性结果仍是假阳性。这就是医生会安排确认检测的原因。
...
一个单词就把概率从 30% 推高到了 95.5%。真正的垃圾邮件过滤器会同时对数百个单词应用贝叶斯定理。
...
这就是 MLE：选择使观测数据最有可能出现的参数值。你最大化的是似然函数；对于离散计数，它可以简化为相对频率。
...
MAP 还为参数本身加入了先验。如果你认为参数应该较小，就可以把这种信念编码成一个惩罚大参数值的先验。这与 ML 中的 L2 正则化完全相同。岭回归中的“ridge”惩罚，从本质上说就是权重上的高斯先验。
...
**先验就是正则化。** 权重上的高斯先验就是 L2 正则化，Laplace 先验就是 L1。每当你添加一个正则化项，实际上都在对自己预期哪些参数值作出贝叶斯式陈述。
