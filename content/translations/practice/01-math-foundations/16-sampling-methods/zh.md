...
生成式 AI 系统依靠抽样产生输出，抽样策略决定质量、多样性与可控性。本课从均匀随机数出发，逐步实现现代 LLM 和生成模型使用的主要抽样方法。
...
一个均匀随机数包含从任意分布生成样本所需的随机性；剩下的问题是找到正确的变换。
...
界 M 越紧，接受率越高。在低维（1–3 维）中，拒绝抽样效果良好；在高维中，因提议分布的大部分体积都会被拒绝，接受率会指数级下降，这种现象称为拒绝抽样中的维度灾难。
...
对于对称提议分布（q(x'|x) = q(x|x')），该比值可化简为 p(x')/p(x)，得到原始的 Metropolis 算法。
...
重参数化之所以可行，是因为 N(mu, sigma^2) 与 mu + sigma * N(0, 1) 具有相同的分布。它把随机性移至一个不含参数的来源（epsilon），再将样本表示为参数的可微变换。
