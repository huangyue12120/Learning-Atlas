...
这不是风格偏好，而是编译器约束。JIT 需要纯函数——相同输入始终产生相同输出、无副作用；正是该限制带来 100 倍加速的可能。
...
控制流限制是真实的：`jax.lax.cond` 替代 if/else，`jax.lax.scan` 替代 for 循环。这不是可选项，而是编译的代价。
...
这就是手动完成的 He 初始化：从一个种子拆出三把 PRNG key，每个权重都是嵌套字典中的不可变数组。
