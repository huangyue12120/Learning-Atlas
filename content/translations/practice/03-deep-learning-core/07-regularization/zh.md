...
这属于过拟合；模型越大，问题越严重。GPT-3 有 1750 亿参数，训练集约有 5000 亿个词元。如此多的参数足以让模型逐字记住训练数据中的大量片段。没有正则化，它只会复述训练示例，而非学习可泛化的模式。
...
BatchNorm 为何有效仍有争议。原论文称它减少了“内部协变量偏移”（较早层更新时，层输入分布发生变化）。Santurkar 等人（2018）表明这一解释不成立，并发现 BatchNorm 会让损失景观更平滑、梯度更可预测，同时减小 Lipschitz 常数，因此优化器可以采用更大的安全步长和更高学习率，加快收敛。
...
LLaMA、LLaMA 2、LLaMA 3、Mistral 和大多数现代 LLM 都使用 RMSNorm 而不是 LayerNorm。参数达到数十亿、词元达到数万亿时，这 10% 的节省相当重要。
...
必须正确切换 `model.train()` / `model.eval()`：它会开关 dropout，并通知 BatchNorm 使用批统计量还是滑动统计量。推理前忘记调用 `model.eval()` 是深度学习最常见的错误之一；此时 dropout 仍在生效，BatchNorm 仍使用小批次统计量，测试准确率会随机波动。
