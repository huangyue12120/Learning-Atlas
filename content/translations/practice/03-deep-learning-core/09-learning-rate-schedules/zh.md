...
近三年发表的大型模型普遍使用学习率调度。Llama 3 使用峰值 lr=3e-4、2000 步预热、余弦衰减至 3e-5；GPT-3 使用 lr=6e-4，并在 3.75 亿词元内预热。这些配置来自耗资数百万美元的大规模超参数扫描。
...
### 预热：为什么要从小开始 <!-- learning-atlas: warmup-why-you-start-small -->
...
典型预热占总训练步数 1–5%。Llama 3 在约 1.8 万亿词元上训练、预热 2000 步；GPT-3 在 3.75 亿词元上预热。
