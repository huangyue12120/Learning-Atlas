...
1969 年 Minsky 和 Papert 证明，单层网络在数学上无法学习 XOR：`[0,1]`、`[1,0]` 在一侧，`[0,0]`、`[1,1]` 在另一侧，单条直线不能将它们分开。这项结果使神经网络研究资金停滞十多年。

增加隐藏层即可突破这一限制。第一层将输入空间转换为新特征，第二层再组合这些特征，形成单条直线无法表达的决策。多层网络由此成为现代深度学习模型的基础。首先需要实现前向传播，即让数据从输入经过隐藏层到达输出。
...
**输入层** 不执行计算，只保存原始数据；两项特征即两个输入节点。**隐藏层** 完成变换：每个神经元接收前层全部输出、加权加偏置后激活，“隐藏”是因为训练数据中不直接包含其值。**输出层** 给出最终答案；二分类用一个 sigmoid 神经元，多分类每类一个。
...
这不表示一层总是最佳，仅表示架构理论上有能力。实践中更深的网络（更多层、每层更少神经元）以远少于浅宽网络的参数学习同样函数，这解释了深度学习的有效性。
