...
> 存下全部数据，再根据最近邻居作出预测；简单的规则也能形成有效基线。
...
它没有训练阶段、可学习参数或需要最小化的损失函数；只存储完整训练集，在预测时计算距离。尽管规则简单，它在中小数据集上仍有竞争力，也能说明距离度量选择、维度灾难和惰性/急切学习等基础概念。向量数据库用 KNN 搜索嵌入，RAG 查找最近文档块，推荐系统查找相似用户或物品；算法相同，差别只在规模和数据结构。
