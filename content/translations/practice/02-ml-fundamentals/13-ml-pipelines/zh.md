...
这些问题经常导致 ML 系统在生产中失败。管道将各个转换步骤打包成单个有序且可复现的对象，从而保持训练与推理过程一致。
...
### 数据泄漏：沉默的杀手 <!-- learning-atlas: data-leakage-the-silent-killer -->
...
生产环境通常应为 OneHotEncoder 设置 `handle_unknown="ignore"`。出现新类别（模型未见过的城市）时，它生成零向量而非崩溃。
