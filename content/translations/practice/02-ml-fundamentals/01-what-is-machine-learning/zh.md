...
自监督融合监督和无监督思想，严格地说仍属于监督学习（模型在预测某物），只是标签由数据自动生成，而非人工提供。
...
### 机器学习工作流 <!-- learning-atlas: the-ml-workflow -->
...
测试集只应在最终评估时使用一次。若持续根据测试表现调模型，就会把测试信息带入训练决策，使报告的数字失去意义。小数据集可使用 k 折交叉验证：分成 k 份，用 k−1 份训练、剩余一份验证，轮换后取平均。
...
完整算法只有两步：`fit` 计算两个均值，`predict` 计算距离；没有梯度下降、迭代或超参数。
