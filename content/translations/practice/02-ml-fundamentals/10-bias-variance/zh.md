...
如果模型太简单（例如在弯曲数据集上做线性回归），它会持续错过真实模式，这就是**偏差**。如果模型太复杂（例如用 15 个数据点拟合 20 次多项式），它能完美拟合训练数据，却会在新数据上给出变化剧烈的预测，这就是**方差**。
...
关键洞见：如果两条曲线都已平台期，间隙小但两种误差都高，更多数据毫无用处，你需要更好的模型；若间隙大且仍在缩小，更多数据会有帮助。
