...
路径禁区是最微妙的部分。沙箱构造时接收一个 project_root。任何看起来像路径的参数（包含 / 或匹配已有条目）都会先经过 os.path.realpath 规范化，然后与项目根目录的 realpath 比较。如果解析后的目标不在根目录下，就拒绝。即使符号链接名称不含斜杠，只要它存在于项目根目录中，也会经过这项检查；`os.path.lexists` 还会识别目标不存在的悬空符号链接。通过检查 realpath，而不是字面路径，可以阻止项目根内指向外部位置的符号链接逃逸。
