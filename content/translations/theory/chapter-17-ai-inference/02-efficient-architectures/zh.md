...
## Linear Attention and 状态-Space Models

- Can we replace the $O(n^2)$ attention entirely? **Linear attention** and **状态-space models (SSMs)** process sequences in $O(n)$ time by avoiding the explicit attention matrix.
...
- **Mamba** (Gu & Dao, 2023) is a selective 状态-space model. It processes sequences through learned 状态 transitions:
...
- Flash Attention is now the default attention implementation in PyTorch (`torch.nn.functional.scaled_dot_product_attention`), JAX, and all major inference 框架s. If you are running attention in 2024+, you are almost certainly using Flash Attention.
