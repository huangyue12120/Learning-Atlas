...
*Serving large models to millions of users requires distributing inference across multiple GPUs, predicting tokens before they are needed, caching shared context, and choosing the right 框架. This file covers parallelism at inference, speculative decoding, prefix caching, inference 框架s, cost optimisation, and monitoring.*
...
- **Variants**:
    ...
    - **EAGLE** (Li et al., 2024): trains a lightweight draft head that uses the target model's hidden 状态s to predict future tokens. Higher acceptance rate than independent draft models.
...
- The LLM serving ecosystem has converged on a few dominant 框架s:

| 框架 | Strengths | Best For |
