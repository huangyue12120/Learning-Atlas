...
const mermaidLabelTerms = [
  ["Training Data", "训练数据"], ["Validation Data", "验证数据"], ["Test Data", "测试数据"],
  ["Raw Data", "原始数据"], ["Raw Text", "原始文本"], ["Full Dataset", "完整数据集"],
  ["Machine Learning", "机器学习"], ["Learning Algorithm", "学习算法"], ["Expected Output", "期望输出"],
  ["Supervised Learning", "监督学习"], ["Unsupervised Learning", "无监督学习"], ["Reinforcement Learning", "强化学习"],
  ["Dimensionality Reduction", "降维"], ["Policy Optimization", "策略优化"], ["Value Learning", "价值学习"],
  ["Collect Data", "收集数据"], ["Clean & Explore", "清洗与探索"], ["Feature Engineering", "特征工程"],
  ["Split Data", "划分数据"], ["Train Model", "训练模型"], ["Tune Hyperparameters", "调优超参数"], ["Tune hyperparameters", "调优超参数"],
  ["Final Evaluation", "最终评估"], ["Final evaluation", "最终评估"], ["Final Model", "最终模型"], ["Final model", "最终模型"], ["Report Performance", "报告性能"],
  ["Not good enough", "效果不够好"], ["Good enough", "效果足够好"], ["Performance drops", "性能下降"],
  ["Initialize w and b randomly", "随机初始化 w 和 b"], ["Compute predictions", "计算预测"],
  ["Compute cost", "计算代价"], ["Compute gradients", "计算梯度"], ["Update parameters", "更新参数"],
  ["Cost low enough?", "代价是否足够低？"], ["Done: optimal w and b found", "完成：找到最优 w 和 b"],
  ["Input features", "输入特征"], ["Predict Probabilities", "预测概率"], ["Predict 1", "预测为 1"], ["Predict 0", "预测为 0"],
  ["Initialize w=0, b=0", "初始化 w=0、b=0"], ["Forward pass", "前向传播"], ["Compute loss", "计算损失"], ["binary cross-entropy", "二元交叉熵"], ["Converged?", "是否收敛？"],
  ["Decision Boundary", "决策边界"], ["Classification", "分类"], ["Regression", "回归"], ["Clustering", "聚类"],
  ["K-Means", "K-Means 聚类"], ["DBSCAN", "DBSCAN 聚类"], ["GMM", "高斯混合模型（GMM）"], ["Hierarchical", "层次聚类"], ["Flat, spherical clusters", "扁平、球形簇"],
  ["Arbitrary shapes, noise detection", "任意形状、噪声检测"], ["Tree of nested clusters", "嵌套簇的树"],
  ["Soft assignments, elliptical clusters", "软分配、椭圆簇"], ["Choose Method", "选择方法"],
  ["Handle Missing Values", "处理缺失值"], ["Numerical Transforms", "数值变换"], ["Categorical Encoding", "类别编码"],
  ["Text Features", "文本特征"], ["Feature Interactions", "特征交互"], ["Feature Selection", "特征选择"],
  ["Model-Ready Data", "可供模型使用的数据"], ["Train Set", "训练集"], ["Validation Set", "验证集"], ["Test Set", "测试集"],
  ["Fit Model", "拟合模型"], ["Tune Hyperparameters", "调优超参数"], ["Simple Model", "简单模型"],
  ["Sweet Spot", "最佳平衡点"], ["Complex Model", "复杂模型"], ["Increase complexity", "提高复杂度"], ["increase complexity", "提高复杂度"], ["Too much complexity", "复杂度过高"], ["Underfit Zone", "欠拟合区"],
  ["Classical Sweet Spot", "经典最佳平衡点"], ["Interpolation Threshold", "插值阈值"],
  ["Double Descent - Error Drops Again", "双降现象：误差再次下降"], ["High variance - overfitting", "高方差：过拟合"],
  ["High bias - underfitting", "高偏差：欠拟合"], ["Good fit", "良好拟合"], ["Compare train error vs test error", "比较训练误差与测试误差"],
  ["Large gap?", "差距很大？"], ["Both errors high?", "两个误差都高？"], ["Gap between train and val?", "训练与验证之间差距？"],
  ["More data / Regularize / Simpler model", "更多数据 / 正则化 / 更简单模型"], ["More features / Complex model / Less regularization", "更多特征 / 更复杂模型 / 更少正则化"],
  ["Large gap, val still decreasing", "差距大，验证误差仍在下降"], ["Small gap, both high", "差距小，两个误差都高"], ["Large gap, val flat", "差距大，验证误差持平"],
  ["Try more complex model", "尝试更复杂模型"], ["More data will help", "更多数据会有帮助"],
  ["More data will NOT help", "更多数据没有帮助"], ["Regularize or simplify", "正则化或简化模型"],
  ["Generate learning curve", "生成学习曲线"], ["Generate validation curve", "生成验证曲线"],
  ["Training Data", "训练数据"], ["Bootstrap Sample", "自助采样"], ["Average or Majority Vote", "平均或多数投票"],
  ["Final Prediction", "最终预测"], ["Find errors", "找出错误"], ["Increase weights on errors", "提高错误样本权重"],
  ["Weighted sum of all models", "全部模型的加权和"], ["Meta-Learner", "元学习器"],
  ["Define search space", "定义搜索空间"], ["Evaluate initial random points", "评估初始随机点"],
  ["Fit surrogate model to results", "对结果拟合代理模型"], ["Use acquisition function to pick next point", "用采集函数选择下一点"],
  ["Evaluate the model at that point", "在该点评估模型"], ["Return best hyperparameters found", "返回找到的最佳超参数"],
  ["Budget exhausted?", "预算是否耗尽？"], ["Start with defaults", "从默认值开始"], ["Coarse random search", "粗略随机搜索"], ["Fine random or Bayesian search", "精细随机或贝叶斯搜索"], ["Final model with best hyperparameters", "采用最佳超参数的最终模型"],
  ["Identify important hyperparameters", "识别重要超参数"], ["Retrain on full training data", "在全部训练数据上重训"],
  ["3 unique learning rates", "3 个不同学习率"], ["3 unique max depths", "3 个不同最大深度"], ["9 total evaluations", "共 9 次评估"], ["9 unique learning rates", "9 个不同学习率"], ["9 unique max depths", "9 个不同最大深度"],
  ["Inner 5-fold CV on remaining data", "在剩余数据上做内层 5 折交叉验证"], ["Best hyperparams for fold", "该折的最佳超参数"],
  ["Impute Missing Values", "填补缺失值"], ["Scale Numeric Features", "缩放数值特征"],
  ["Encode Categoricals", "编码类别特征"], ["Prediction", "预测"], ["Jupyter Notebook", "Jupyter Notebook"], ["Add config file for hyperparameters", "添加超参数配置文件"],
  ["Extract functions", "提取函数"], ["Build Pipeline object", "构建 Pipeline 对象"], ["Add experiment tracking", "加入实验追踪"],
  ["Add data validation", "加入数据验证"], ["Add tests", "加入测试"], ["Package for deployment", "打包部署"],
  ["Build Vocabulary", "构建词表"], ["Tokenize", "分词"], ["Count Word Frequencies", "统计词频"], ["Apply Smoothing", "应用平滑"],
  ["Compute Log Probabilities", "计算对数概率"], ["Predict: argmax P class given words", "预测：给定词语时取 P(类别) 最大者"],
  ["Sample 1", "样本 1"], ["Sample 2", "样本 2"], ["Sample 3", "样本 3"],
  ["Observed Time Series", "观测时间序列"], ["Trend", "趋势"], ["Seasonality", "季节性"],
  ["Long-term direction", "长期趋势"], ["Repeating patterns", "重复模式"], ["Residual/Noise", "残差/噪声"],
  ["Anomaly Types", "异常类型"], ["Point Anomaly", "点异常"], ["Contextual Anomaly", "上下文异常"],
  ["Collective Anomaly", "集体异常"], ["All Data Points", "全部数据点"], ["Left Partition", "左分区"],
  ["Right Partition", "右分区"], ["Normal Point - deep in tree", "正常点：位于树的深处"],
  ["More splits needed...", "需要更多切分…"], ["Random Feature + Random Split", "随机特征 + 随机切分"],
  ["Train on Normal Data Only", "仅用正常数据训练"], ["Score All Test Data", "为全部测试数据评分"],
  ["Rank by Anomaly Score", "按异常分数排序"], ["Evaluate Top-K Flagged Items", "评估前 K 个标记项目"],
  ["Precision at K / AUPRC", "Precision@K / AUPRC"], ["Imbalanced Dataset", "不平衡数据集"],
  ["Class Weights", "类别权重"], ["Threshold Tuning", "阈值调优"], ["Try Different Strategy", "尝试其他策略"],
  ["Deploy with Monitoring", "部署并监控"], ["Imbalance Ratio?", "不平衡比例？"], ["Mild:", "轻度："], ["Moderate:", "中度："], ["Severe:", "重度："], ["Good Enough?", "是否足够好？"],
  ["Sweep Thresholds", "扫描阈值"], ["Compute F1 at Each", "计算各阈值的 F1"], ["Pick Best Threshold", "选择最佳阈值"],
  ["Use in Production", "用于生产环境"], ["Start: Imbalanced Dataset", "开始：不平衡数据集"], ["How imbalanced?", "不平衡程度？"], ["Enough data?", "数据足够吗？"], ["Recall high enough?", "召回率足够高吗？"], ["Precision acceptable?", "精确率可接受吗？"],
  ["Lower threshold", "降低阈值"], ["Raise threshold or add features", "提高阈值或增加特征"], ["Ship it", "交付"],
  ["Feature Selection Methods", "特征选择方法"], ["Filter Methods", "过滤方法"], ["Wrapper Methods", "包装方法"],
  ["Embedded Methods", "嵌入式方法"], ["Feature X", "特征 X"], ["Discretize into Bins", "离散为分箱"],
  ["Select Top K", "选择前 K 项"], ["Start: Feature Selection", "开始：特征选择"],
  ["Validate: compare selected vs all features", "验证：比较已选与全部特征"], ["How many features?", "有多少特征？"],
  ["Using linear model?", "使用线性模型？"], ["Performance improved?", "性能是否提升？"],
  ["Input", "输入"], ["Output", "输出"], ["Hidden", "隐藏"], ["Loss", "损失"], ["Problem", "问题"],
  ["DataLoader", "数据加载器"], ["Sequential", "顺序容器"], ["Linear", "线性层"], ["Rules", "规则"],
  ["Program", "程序"], ["Data", "数据"], ["Model", "模型"], ["Deploy", "部署"], ["Monitor", "监控"],
  ["Evaluate", "评估"], ["Train", "训练"], ["Validation", "验证"], ["Test", "测试"], ["No", "否"], ["Yes", "是"]
];

function localizeMermaidLabel(label) {
  let localized = label;
  for (const [english, chinese] of mermaidLabelTerms) localized = localized.replaceAll(english, chinese);
  return localized;
}

function localizeMermaidSource(source) {
  if (!/^(?:flowchart|graph)\b/m.test(source)) return source;
  return source
    .replace(/"([^"\n]*)"/g, (_, label) => `"${localizeMermaidLabel(label)}"`)
    .replace(/\b([A-Za-z][\w-]*)\[([^"\]\n][^\]\n]*)\]/g, (_, id, label) => `${id}[${localizeMermaidLabel(label)}]`)
    .replace(/\b([A-Za-z][\w-]*)\{([^"}\n][^}\n]*)\}/g, (_, id, label) => `${id}{${localizeMermaidLabel(label)}}`)
    .replace(/\|([^|\n]+)\|/g, (_, label) => `|${localizeMermaidLabel(label)}|`);
}
...
function mermaidDiagram(source) {
  ...
  const localizedSource = localizeMermaidSource(source);
  return `<figure class="concept-diagram"><div class="mermaid-diagram" data-mermaid-id="${id}" data-mermaid-source="${encodeURIComponent(localizedSource)}" aria-busy="true"><p>正在生成图示…</p></div><figcaption>课程图示</figcaption></figure>`;
}
