...
function renderMarkdown(markdown, cards) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (line.startsWith("|") && /^\|[-| :]+\|$/.test(lines[index + 1] ?? "")) {
      flushParagraph();
      closeList();
      const readCells = (row) => row.slice(1, -1).split("|").map((cell) => inlineMarkdown(cell.trim()));
      const headers = readCells(line);
      index += 1;
      const rows = [];
      while (index += 1, index < lines.length && lines[index].startsWith("|")) rows.push(readCells(lines[index]));
      index -= 1;
