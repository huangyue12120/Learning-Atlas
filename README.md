...
当前发布版本为 **v0.22.0**（2026-09-17）：首页展示 Phase 0-19 的完整实践地图与 20 章、104 篇笔记的完整理论目录。实践阅读器现已开放 Phase 0-17 的 408 节课程，Phase 18-19 的 115 节内容显示为“已审核，待接入”；当前开放课程的 284 张上下文理论卡已提供可展开的直觉、关键点、应用和自检问题，57 / 104 篇审核中文理论全文已覆盖 311 / 311 张理论卡，所有理论原文入口均打开官方上游最新 `main`。内容仓库维护 523 节审核中文实践译文、258 份中文测验、400 套 VS Code/Python 工作区模板与练习指南，以及 311 条人工批准的理论关联。P17 新增的 28 份中文测验、28 套 Python 工作区和 27 张 rich theory cards 已通过结构、来源指纹、API 和回退门禁；P16 的 25 节课程及其两个 TypeScript 例外继续保持已发布状态。
...
- 目前实践阅读器发布 Phase 0-17 的 408 节经审核实践主线和已批准理论卡；其中 400 节提供 VS Code/Python 工作区模板与练习指南，P16 的 01、03 课保留 TypeScript 上游实现链接。Phase 18-19 的 115 节译文已审核但尚未接入阅读器。完整理论目录已经开放；只有审核且来源指纹与本地 `main` 快照匹配的中文正文会在应用内发布，理论原文链接始终指向可能变化的官方 `main`。当前已有 57 / 104 篇中文理论全文进入应用内发布，覆盖 311 / 311 条理论卡关联；剩余 47 篇为尚未被理论卡引用的独立笔记。
...
发布信息、已知限制和备份建议请参阅 [v0.22.0 发布说明](docs/release-notes.md)。
