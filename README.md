...
当前发布版本为 **v0.21.0**（2026-09-16）：首页展示 Phase 0-19 的完整实践地图与 20 章、104 篇笔记的完整理论目录。实践阅读器现已开放 Phase 0-16 的 380 节课程，Phase 17-19 的 143 节内容显示为“已审核，待接入”；当前开放课程的 257 张上下文理论卡已提供可展开的直觉、关键点、应用和自检问题，57 / 104 篇审核中文理论全文已覆盖 311 / 311 张理论卡，所有理论原文入口均打开官方上游最新 `main`。内容仓库维护 523 节审核中文实践译文、230 份中文测验、372 套 VS Code/Python 工作区模板与练习指南，以及 311 条人工批准的理论关联。T4 新增的 19 篇关联长尾理论已通过结构、来源指纹、API 和回退门禁；P16 的 25 节课程、2 份中文测验、23 套 Python 工作区和 2 个 TypeScript 例外继续保持已发布状态。
...
1. 从首页开始或继续实践主线，也可展开 20 章理论目录独立阅读。新学习者从 Phase 0 第一课开始；阅读器当前开放 Phase 0-16，共 380 节课，Phase 17-19 仅展示为已审核、待接入。
...
- 目前实践阅读器发布 Phase 0-16 的 380 节经审核实践主线和已批准理论卡；其中 372 节提供 VS Code/Python 工作区模板与练习指南，P16 的 01、03 课保留 TypeScript 上游实现链接。Phase 17-19 的 143 节译文已审核但尚未接入阅读器。完整理论目录已经开放；只有审核且来源指纹与本地 `main` 快照匹配的中文正文会在应用内发布，理论原文链接始终指向可能变化的官方 `main`。当前已有 57 / 104 篇中文理论全文进入应用内发布，覆盖 311 / 311 条理论卡关联；剩余 47 篇为尚未被理论卡引用的独立笔记。
...
发布信息、已知限制和备份建议请参阅 [v0.21.0 发布说明](docs/release-notes.md)。
