...
当前发布版本为 **v0.26.1**（2026-09-25）：首页展示 Phase 0-19 的完整实践地图与 20 章、104 篇笔记的完整理论目录。实践阅读器现已开放 Phase 0-18 的 438 节课程，Phase 19 的 85 节内容显示为“已审核，待接入”；当前开放课程的 289 张上下文理论卡已提供可展开的直觉、关键点、应用和自检问题，89 / 104 篇审核中文理论全文已覆盖 311 / 311 张理论卡，所有理论原文入口均打开官方上游最新 `main`。内容仓库维护 523 节审核中文实践译文、288 份中文测验、430 套 VS Code/Python 工作区模板与练习指南，以及 311 条人工批准的理论关联。P18 新增的 30 份中文测验、30 套 Python 工作区和 5 张 rich theory cards 已通过结构、来源指纹、API 和回退门禁；T1–T6 的 89 篇理论正文与 255 个中文 SVG 图示已完成复核。本版按 Issue #14 将实践快照更新到 `0285d9bd92bc95d56ba79bed2071be6fb3365369`，修订 3 篇正文变更对应的译文，并对齐 Phase 10、16、19 的中文示例。本版理论复核记录见[理论复核清单](docs/theory-translation-review.md)与[术语表](docs/translation-glossary.md)。P16 的 25 节课程及其两个 TypeScript 例外继续保持已发布状态。
...
1. 从首页开始或继续实践主线，也可展开 20 章理论目录独立阅读。新学习者从 Phase 0 第一课开始；阅读器当前开放 Phase 0-18，共 438 节课，Phase 19 仅展示为已审核、待接入。
...
前四项分别核对译文与原文的结构对应关系、全部版本化内容的来源 SHA-256、已发布理论卡的内容契约，以及是否误提交学习数据、私钥、凭据、令牌和本地状态文件；后两项检查浏览器模块与服务端语法，并运行应用 API 测试。翻译覆盖率由每周运行的监控 workflow 检查。
...
- 目前实践阅读器发布 Phase 0-18 的 438 节经审核实践主线和已批准理论卡；其中 430 节提供 VS Code/Python 工作区模板与练习指南，P16 的 01、03 课保留 TypeScript 上游实现链接。Phase 19 的 85 节译文已审核但尚未接入阅读器。完整理论目录已经开放；只有审核且来源指纹与本地 `main` 快照匹配的中文正文会在应用内发布，理论原文链接始终指向可能变化的官方 `main`。当前已有 89 / 104 篇中文理论全文进入应用内发布，覆盖 311 / 311 条理论卡关联；剩余 15 篇为尚未被理论卡引用的独立笔记。
...
根项目原创软件代码（包括本地应用、脚本、测试和配置）采用 [Apache-2.0](LICENSE)。原创文档、设计材料和独立中文学习内容采用 [CC BY 4.0](LICENSE-CONTENT.md)。上游衍生译文和代码片段仍分别受上游 MIT 或 Apache-2.0 约束，并保留归属、来源指纹与变更说明；字体、npm 依赖和其他第三方资产继续使用各自许可证。已知边界和再分发提示见 [第三方归属清单](THIRD_PARTY_NOTICES.md)；这些文件不替代各上游或第三方的完整法律文本。

---

发布信息、已知限制和备份建议请参阅 [v0.26.1 发布说明](docs/release-notes.md)。
