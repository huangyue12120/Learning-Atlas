...
此表记录 v0.22.0 的可复现发布快照。两个 submodule 同时配置为跟踪其上游 `main` 分支：每日检查只报告待审核更新，不会自动修改发布内容。P17 在同一实践快照上接入 28 节基础设施与生产课程，新增 28 份中文测验、28 套 Python 工作区和 27 张 rich theory cards；中文改编、测验和理论关联的各自 front matter 均记录对应原文路径、revision 与 SHA-256；中文理论正文还声明跟踪 `main`。应用发现实践课程、测验或中文理论正文的当前 SHA-256 不匹配时，会拒绝发布旧内容；理论原文链接则始终指向官方 `main`。
...
理论上游快照包含 20 章、104 篇笔记，已全部进入首页理论目录。理论关联共 311 条，已全部完成人工审核并标为 `approved`；当前开放课程的 284 张已批准关联已迁移到结构化上下文卡片并绑定到对应的中文课程标题，展开后提供直觉、关键点、应用和自检问题，尚未接入阅读器的 27 张关联仍保留在 Phase 18–19 的迁移范围内。P17 新增 27 张卡，覆盖推理基础设施、可观测性、合规、FinOps 与生产治理；它们与既有卡一样通过实践锚点和来源指纹门禁。T1 新增 11 篇高关联中文理论，连同既有《抽样》、T2 新增 13 篇、T3 新增 13 篇和 T4 新增 19 篇关联理论，共 57 / 104 篇中文理论全文，对应 311 / 311 张理论卡进入应用内中文阅读。应用还会对相同的理论目标去重，并在存在中文全文时保留不同目标的英文入口。第 15 课的中心极限定理卡指向稳定标识 `theory/chapter-04-statistics/03-sampling`，并进入对应的版本化中文全文。

当前已有 57 篇中文理论正文满足 `status: reviewed`、来源路径和 SHA-256 三重发布条件，包括既有《抽样》、T1 的 11 篇高关联笔记、T2 的 13 篇、T3 的 13 篇和 T4 的 19 篇关联长尾笔记。它们是针对本地同步到 `main` 的快照审核的版本化中文内容；审核 revision 仅用于追溯，目录和阅读器中的原文入口均指向官方 `HenryNdubuaku/maths-cs-ai-compendium` 最新英文 `main`。其余 47 篇理论笔记从目录打开同一官方 `main`，属于可能变化的外部实时参考，不计入本地发布快照。
...
结构检查覆盖 523 份实践译文与 57 份版本化理论译文；来源指纹检查覆盖 1460 条记录；理论卡契约与实践锚点检查覆盖当前发布的 284 张卡；仓库卫生检查阻止学习数据、本地状态、凭据、私钥和常见令牌误入版本库。Python-first 例外必须在原位置明确链接到锁定的上游非 Python 实现。
...
- **CI / Validate content and application** 在每次推送和面向 `main` 的 PR 中检查译文结构、1460 项来源指纹、仓库卫生、前端/服务端语法和应用 API 测试。同步 PR 必须在内容更新后通过此检查才能合入。
...
- 完整理论目录已经开放；当前 57 / 104 篇中文理论全文（包括 T1 的 11 篇高关联笔记、T2 的 13 篇、T3 的 13 篇、T4 的 19 篇关联长尾笔记与《抽样》）已通过审核并在应用内发布，其余 47 篇理论笔记仍以官方 `main` 英文原文为阅读目标。理论卡和中文理论阅读器的原文入口均跟随官方 `main`。
