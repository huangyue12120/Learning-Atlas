...
此表记录 v0.25.0 的可复现发布快照。两个 submodule 同时配置为跟踪其上游 `main` 分支：每日检查只报告待审核更新，不会自动修改发布内容。P18 在同一实践快照上接入 30 节伦理、安全与对齐课程，新增 30 份中文测验、30 套 Python 工作区和 5 张 rich theory cards；T5 和 T6 在同一理论快照上各新增 16 篇按章节顺序复核的独立理论全文。中文改编、测验和理论关联的各自 front matter 均记录对应原文路径、revision 与 SHA-256；中文理论正文还声明跟踪 `main`。应用发现实践课程、测验或中文理论正文的当前 SHA-256 不匹配时，会拒绝发布旧内容；理论原文链接则始终指向官方 `main`。
