...
| --- | --- | --- |
| `ai-engineering-from-scratch` | `0285d9bd92bc95d56ba79bed2071be6fb3365369` | MIT |
| `maths-cs-ai-compendium` | `9850ee574a370bc1cde59de98b394e953775b67d` | Apache-2.0 |

此表记录 v0.26.1 的可复现发布快照；实践源从 v0.26.0 的 `d18b8fe5a913c46011a3b06cb6ebd6a924414fd3` 更新至 `0285d9bd92bc95d56ba79bed2071be6fb3365369`，理论源保持 `9850ee574a370bc1cde59de98b394e953775b67d`，两个上游仍继续跟踪各自的 `main` 分支。每日检查只报告待审核更新，不会自动修改发布内容。本版依 Issue #14 审核上游变化：更新 3 篇正文源有变化的译文和 2 条理论关联的实践来源指纹，并同步修订 Phase 10、16、19 的代码示例；复核后保留两条理论关联，并澄清一条卡片摘要中的状态表述。其余改动未命中当前阅读器发布范围的中文正文、测验或理论目标。P18 在 v0.24.0 接入 30 节伦理、安全与对齐课程；T5 和 T6 在理论快照上各新增 16 篇按章节顺序复核的独立理论全文，v0.25.2 补齐 T1–T6 全部 89 篇理论正文及其中文 SVG 图示，v0.26.0 完成 89 篇中英一致性、术语和中文表达复核，逐篇 SHA-256 与复核要点见[理论复核清单](theory-translation-review.md)。中文改编、测验和理论关联的各自 front matter 均记录对应原文路径、revision 与 SHA-256；中文理论正文还声明跟踪 `main`。应用发现实践课程、测验或中文理论正文的当前 SHA-256 不匹配时，会拒绝发布旧内容；理论原文链接则始终指向官方 `main`。
...
上游实践源现有 Phase 0-19 共 523 节课程；内容仓库现有 523 节经审核中文实践课。首页展示全部 20 个 Phase；实践阅读器开放 Phase 0-18 的 438 节，Phase 19 的 85 节只显示“已审核，待接入”。Phase 15、Phase 16、Phase 17 和 Phase 18 的课程已全部进入当前开放范围。
...
| Phase 16「多智能体与群体」 | 25 | 2 | 已开放 |
| Phase 17「基础设施与生产」 | 28 | 28 | 已开放 |
| Phase 18「伦理、安全与对齐」 | 30 | 30 | 已开放 |
| Phase 19「综合项目」 | 85 | 0 | 已审核，待接入 |
| **合计** | **523** | **288** | — |

当前开放的 438 节课程中，有 430 节提供 VS Code/Python 工作区模板与练习指南；其余 8 节没有 Python 实作资源，其中 P16 的 01、03 课明确保留 TypeScript 上游实现。中文测验覆盖当前开放范围内所有存在上游 `quiz.json` 的课程；待接入范围另有 85 份上游 `quiz.json`，其余上游课程没有 `quiz.json`，不属于待补译缺口。Phase 15 的 22 节课程上游均没有 `quiz.json`，因此不凭空创建测验。

理论上游快照包含 20 章、104 篇笔记，已全部进入首页理论目录。理论关联共 311 条，已全部完成人工审核并标为 `approved`；当前开放课程的 289 张已批准关联已迁移到结构化上下文卡片并绑定到对应的中文课程标题，展开后提供直觉、关键点、应用和自检问题，尚未接入阅读器的 22 张关联仍保留在 Phase 19 的迁移范围内。P18 新增 5 张卡，覆盖对齐训练、提示注入和红队安全评估；它们与既有卡一样通过实践锚点和来源指纹门禁。T1 新增 11 篇高关联中文理论，连同既有《抽样》、T2 新增 13 篇、T3 新增 13 篇、T4 新增 19 篇关联理论、T5 新增 16 篇和 T6 新增 16 篇独立理论，共 89 / 104 篇中文理论全文，其中 311 / 311 张理论卡关联仍可进入应用内中文阅读。应用还会对相同的理论目标去重，并在存在中文全文时保留不同目标的英文入口。第 15 课的中心极限定理卡指向稳定标识 `theory/chapter-04-statistics/03-sampling`，并进入对应的版本化中文全文。
...
结构检查覆盖 523 份实践译文与 89 份版本化理论译文；来源指纹检查覆盖 1522 条记录；理论卡契约与实践锚点检查覆盖当前发布的 289 张卡；仓库卫生检查阻止学习数据、本地状态、凭据、私钥和常见令牌误入版本库。P18 的 30 个 `code/main.py` 均已逐个运行通过。Python-first 例外必须在原位置明确链接到锁定的上游非 Python 实现。
...
- **CI / Validate content and application** 在每次推送和面向 `main` 的 PR 中检查译文结构、1522 项来源指纹、仓库卫生、前端/服务端语法和应用 API 测试。同步 PR 必须在内容更新后通过此检查才能合入。
...
- 实践阅读器目前加载 Phase 0-18 的 438 节课程；Phase 19 的 85 节译文已审核但尚未接入；上下文理论层仍只呈现经过批准的关联。
- 中文测验目前覆盖当前开放范围内有上游 `quiz.json` 的 288 节课程；待接入范围另有 85 份上游 `quiz.json`，另有 150 节上游课程没有测验文件。
- 完整理论目录已经开放；当前 89 / 104 篇中文理论全文（包括 T1 的 11 篇高关联笔记、T2 的 13 篇、T3 的 13 篇、T4 的 19 篇关联长尾笔记、T5 的 16 篇、T6 的 16 篇独立笔记与《抽样》）已通过审核并在应用内发布，其余 15 篇理论笔记仍以官方 `main` 英文原文为阅读目标。理论卡和中文理论阅读器的原文入口均跟随官方 `main`。
